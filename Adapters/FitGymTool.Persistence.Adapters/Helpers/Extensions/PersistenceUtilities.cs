...
internal static class PersistenceUtilities
{
	...
	/// <summary>
	/// Prepares the update member fees sp parameters.
	/// </summary>
	/// <param name="memberFeesData">The member fees data.</param>
	/// <returns>The sql parameters</returns>
	internal static SqlParameter[] PrepareUpdateMemberFeesSPParameters(this UpdateMemberFees memberFeesData)
	{
		var dataTable = new DataTable();
		dataTable.Columns.Add(StoredProcedures_Inputs.MemberEmail, typeof(string));
		dataTable.Columns.Add(StoredProcedures_Inputs.Amount, typeof(decimal));
		dataTable.Columns.Add(StoredProcedures_Inputs.FromDate, typeof(DateTime));
