...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider adding the 'required' modifier or declaring as nullable.
    ...
    public async Task<List<T>> ExecuteSqlQueryAsync<T>(string sql, params object[] parameters) where T : class, new()
    {
        ...
        {
            return await _dbContext.CurrentMonthFeesAndRevenueStatus.FromSqlRaw(sql, parameters).ToListAsync() as List<T> ?? [];
        }
        else if (typeof(T) == typeof(CurrentMembersFeesStatus))
        {
            return await _dbContext.CurrentMemberFeesStatus.FromSqlRaw(sql, parameters).ToListAsync() as List<T> ?? [];
        }
        else if (typeof(T) == typeof(MemberPaymentHistoryData))
        {
            return await _dbContext.MemberPaymentHistoryData.FromSqlRaw(sql, parameters).ToListAsync() as List<T> ?? [];
        }
